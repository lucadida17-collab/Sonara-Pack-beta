...
exports.handler = async (event) => {
  ...
  const urls = new Set([`${MAIN_ORIGIN}/`, `${MAIN_ORIGIN}/home.html`]);

  try {
    const response = await fetch(`${MAIN_API}/api/packs`, {
    ...
    if (Array.isArray(packs)) {
      for (const pack of packs) {
        ...
        if (artistId) urls.add(`${MAIN_ORIGIN}/app/pages/catalog/artist.html?id=${encodeURIComponent(artistId)}`);
      }
    }
  } catch (error) {
    console.error('[SEO sitemap] catalogue indisponible:', error.message);
  }
