...
function creatorSettingsIcons() { if (window.lucide) window.lucide.createIcons() } function settingsRow(icon, label, section) { return `<button type="button" class="creator-settings-row" data-creator-section="${section}"><span><i data-lucide="${icon}"></i><strong>${label}</strong></span><i data-lucide="chevron-right"></i></button>` } function renderCreatorSettingsHome() { settingsLayout.innerHTML = `<section class="creator-settings-page"><header class="creator-settings-header"><button type="button" class="creator-settings-back" aria-label="Retour"><i data-lucide="arrow-left"></i></button><h1>Paramètres créateur</h1><div></div></header><main class="creator-settings-content"><section class="creator-settings-group"><button type="button" class="creator-settings-row" data-creator-language><span><i data-lucide="languages"></i><strong>Langue</strong></span><i data-lucide="chevron-right"></i></button><button type="button" class="creator-settings-row" data-creator-page="account-creator.html"><span><i data-lucide="circle-user-round"></i><strong>Compte créateur</strong></span><i data-lucide="chevron-right"></i></button>${settingsRow("shield-check", "Règlement créateur", "rules")}${settingsRow("scale", "Pages légales vendeur", "legal")}${settingsRow("landmark", "Paiements et versements", "payments")}${settingsRow("lock-keyhole", "Confidentialité créateur", "privacy")}</section><section class="creator-settings-group"><button type="button" class="creator-settings-row" data-creator-page="support-creator.html"><span><i data-lucide="message-circle"></i><strong>Support</strong></span><i data-lucide="chevron-right"></i></button><button type="button" class="creator-settings-row creator-settings-dashboard"><span><i data-lucide="layout-dashboard"></i><strong>Retour au management</strong></span><i data-lucide="chevron-right"></i></button></section></main></section>`; creatorSettingsIcons(); document.querySelector(".creator-settings-back").addEventListener("click", () => { window.location.href = "../profile-creator.html" }); document.querySelector(".creator-settings-dashboard").addEventListener("click", () => { window.location.href = "/app/pages/creator/dashboard.html?mode=management" }); document.querySelector("[data-creator-language]")?.addEventListener("click", () => { window.location.href = "/app/pages/account/settings/language.html?from=creator" }); document.querySelectorAll("[data-creator-section]").forEach(button => { button.addEventListener("click", () => renderCreatorSettingsSection(button.dataset.creatorSection)) }); document.querySelectorAll("[data-creator-page]").forEach(button => { button.addEventListener("click", () => { window.location.href = button.dataset.creatorPage }) }) } function renderCreatorSettingsSection(sectionKey) { const section = creatorSettingsSections[sectionKey]; if (!section) { renderCreatorSettingsHome(); return } settingsLayout.innerHTML = `<section class="creator-settings-page"><header class="creator-settings-header"><button type="button" class="creator-settings-back" aria-label="Retour"><i data-lucide="arrow-left"></i></button><h1>${section.title}</h1><div></div></header><main class="creator-settings-content creator-settings-document"><p class="creator-settings-intro">${section.intro}</p>${section.groups.map(group => `<article class="creator-document-card"><h2>${group.title}</h2>${group.paragraphs.map(paragraph => `<p>${paragraph}</p>`).join("")}</article>`).join("")}${section.action ? `<a class="creator-settings-action" href="${section.action.href}">${section.action.label}</a>` : ""}</main></section>`; creatorSettingsIcons(); document.querySelector(".creator-settings-back").addEventListener("click", renderCreatorSettingsHome) } renderCreatorSettingsHome();